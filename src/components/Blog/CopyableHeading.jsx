...
const CopyableHeading = ({ title, id }) => {
  ...
  const handleCopy = () => {
    ...
    navigator.clipboard.writeText(textToCopy).then(() => {
      setIsCopied(true);
      setTimeout(() => setIsCopied(false), 10000); // Auto-hide the box after 5 seconds
    });
  };

  const handleDismiss = () => {
    setIsCopied(false); // Hide the box when dismissed
  };
  ...
  return (
    <>
      ...
      {/* Notification Box */}
      {isCopied && (
        <div className="fixed bottom-5 backdrop-blur-lg left-1/2 transform -translate-x-1/2 bg-black bg-opacity-90 text-gray-300 border border-gray-500 rounded-lg h-12 w-[500px] flex items-center justify-between px-4 shadow-lg transition-all duration-300 z-50">
          {/* Left Side: Tick Icon and Message */}
          ...
          {/* Right Side: Cross Icon with Hover Effect */}
